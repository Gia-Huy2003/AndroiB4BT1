...
class TaskDatabaseHelper(private val context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        try {
            ...
            Log.d(TAG, "Bảng $TABLE_TASKS đã được tạo.")
        } catch (e: Exception) {
            Log.e(TAG, "Lỗi khi tạo bảng: ${e.message}")
        }
    }
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if (idIndex != -1 && nameIndex != -1 && descriptionIndex != -1) {
                    ...
                    Log.e(TAG, "Một hoặc nhiều cột không tồn tại trong cursor.")
                }
    ...
    fun copyDatabase() {
        ...
        if (!dbFile.exists()) {
            ...
            try {
                ...
                Log.e(TAG, "Lỗi khi sao chép cơ sở dữ liệu: ${e.message}")
            } finally {
                output.flush()
                output.close()
                input.close()
